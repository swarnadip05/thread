...
export async function parseInventoryZip(
  zipBuffer: Buffer,
): Promise<{ products: ParsedZipProduct[]; summary: { totalProducts: number; totalVariants: number; categoriesFound: string[] } }> {
  let zip: JSZip;
  try {
    zip = await JSZip.loadAsync(zipBuffer);
  } catch {
    throw new HttpError(400, "INVALID_ZIP_ARCHIVE", "The uploaded file is not a valid ZIP archive.");
  }
  ...
  if (manifestFile) {
    ...
    try {
      ...
      throw new HttpError(400, "INVALID_MANIFEST_JSON", "Failed to parse JSON manifest inside the ZIP archive.");
    }
  } else {
    throw new HttpError(
      400,
  ...
  for (let index = 0; index < manifestProducts.length; index += 1) {
    ...
    const audience: ProductAudience = ["men", "women", "unisex", "accessories"].includes(raw.audience)
      ? raw.audience
      : "men";
    ...
    const shortDescription = raw.shortDescription?.trim() || `${title} - Premium quality apparel by ${brand}.`;
    const descriptionHtml = raw.descriptionHtml?.trim() || `<p>${shortDescription}</p>`;
    const status: ProductStatus = ["draft", "active", "inactive", "archived"].includes(raw.status ?? "")
    ...
    for (let imgIndex = 0; imgIndex < imagesToProcess.length; imgIndex += 1) {
      ...
      if (zipImage) {
        ...
        const format = (["jpg", "jpeg", "png", "webp", "avif"].includes(ext) ? ext : "jpg") as ParsedZipProduct["images"][0]["format"];
        const mimeType = (format === "png" ? "image/png" : format === "webp" ? "image/webp" : "image/jpeg") as ParsedZipProduct["images"][0]["mimeType"];
    ...
    // Process variants
    const rawVariants = raw.variants?.length ? raw.variants : [
      { sku: `${slug.toUpperCase()}-S`, colour: "Default", size: "S", mrpPaise: 69900, salePricePaise: 54900, availableStock: 20 },
      { sku: `${slug.toUpperCase()}-M`, colour: "Default", size: "M", mrpPaise: 69900, salePricePaise: 54900, availableStock: 25 },
      { sku: `${slug.toUpperCase()}-L`, colour: "Default", size: "L", mrpPaise: 69900, salePricePaise: 54900, availableStock: 20 },
    ];
...
export async function create100InventoryZipBuffer(): Promise<Buffer> {
  ...
  const targets = [
    { audience: "men" as const, categorySlug: "oversized-t-shirts", fit: "Oversized", price: 69900, mrp: 89900, qty: 25 },
    { audience: "men" as const, categorySlug: "t-shirts", fit: "Regular", price: 54900, mrp: 74900, qty: 20 },
    { audience: "men" as const, categorySlug: "classic-fit-t-shirts", fit: "Classic", price: 59900, mrp: 79900, qty: 15 },
    { audience: "women" as const, categorySlug: "oversized-t-shirts", fit: "Oversized", price: 69900, mrp: 89900, qty: 15 },
    { audience: "women" as const, categorySlug: "t-shirts", fit: "Regular", price: 54900, mrp: 74900, qty: 15 },
    { audience: "accessories" as const, categorySlug: "caps", fit: "Regular", price: 49900, mrp: 69900, qty: 10 },
  ];
