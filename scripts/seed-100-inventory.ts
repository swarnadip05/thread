...
import { parseInventoryZip } from "../apps/api/src/catalogue/zip-importer.js";
...
async function seed100Inventory(): Promise<void> {
  ...
  try {
    ...
    console.error(
      "[WARNING] Could not connect to live MongoDB instance:",
      err instanceof Error ? err.message : err,
    );
    console.log("[INFO] Ensuring 100-product ZIP inventory package is generated for UI upload...");
    const zipPath = await generateInventoryZipFile();
    console.log(
      `[SUCCESS] 100-product ZIP created at ${zipPath}. Upload this via Admin UI (/admin/products -> Import ZIP Inventory).`,
    );
    return;
  }
  ...
  console.log(
    `[INFO] Parsed ${summary.totalProducts} products with ${summary.totalVariants} variants.`,
  );
  ...
  console.log(
    `[SUCCESS] Database populated with ${createdCount} products and ${variantCount} inventory variants across designated categories!`,
  );
