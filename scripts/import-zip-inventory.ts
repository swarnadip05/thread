...
export async function importInventoryZip(zipFilePath?: string): Promise<void> {
  ...
  const apiUrl = process.env.API_URL || `http://localhost:${apiPort}/api/v1/admin/products/import-zip`;
  ...
  try {
    ...
    console.log("[INFO] Server HTTP endpoint not reachable directly; parsing zip locally to verify structure:");
    ...
    console.log(`[VERIFIED] Successfully parsed ${parsed.summary.totalProducts} products with ${parsed.summary.totalVariants} variants into categories: ${parsed.summary.categoriesFound.join(", ")}`);
  }
}
